...
class g {
  constructor(t = {}) {
    this.props = t, this.id = t.id || B(), this.value = this.props.default, this.desktop = this.props.default, this.title = t.title || "";
  }
  destroy() {
  ...
  createInput(t) {
    ...
    this.inputEl = s, s.value = String(t.value || A(t.inputType)), s.type = t.inputType, s.placeholder = t.placeholder || "", s.className = t.inputClassName || "";
    const n = (o) => {
      const l = o.target;
      let r = l.value;
      switch (t.inputType) {
...
class O extends S {
  ...
  draw() {
    ...
    const a = (d) => {
      const m = d.value.split(",").map((x) => parseInt(x.trim()));
      if (m.length !== 3 || m.some(isNaN))
        return d.style.border = "1px solid red", !1;
      const [p, C, v] = m, T = p >= 0 && p <= 255 && C >= 0 && C <= 255 && v >= 0 && v <= 255;
      return d.style.border = T ? "" : "1px solid red", T;
    ...
    return r.oninput = (d) => {
      var p;
      let m = d.target.value.trim();
      a(r) && (this.value = m, (p = this.onChange) == null || p.call(this, m), u.value = this.rgbToHex(m), o.style.backgroundColor = `rgb(${m})`);
    }, u.oninput = (d) => {
      var C, v;
      const m = d.target.value, p = this.hexToRgb(m);
  ...
  draw() {
    const t = (s) => {
      this.props.minValue !== void 0 && (s.min = String(this.props.minValue)), this.props.maxValue !== void 0 && (s.max = String(this.props.maxValue)), this.props.className && s.classList.add(this.props.className), s.addEventListener("input", () => {
        const n = this.props.minValue ?? Number.MIN_SAFE_INTEGER, a = this.props.maxValue ?? Number.MAX_SAFE_INTEGER;
        let o = Number(s.value);
        o < n && (o = n), o > a && (o = a), String(o) !== s.value && (s.value = String(o)), this.setValue(o);
      }), s.addEventListener("blur", () => {
        var a, o;
        const n = this.validateValue(Number(s.value));
        n !== Number(s.value) && (s.value = String(n), this.setValue(n)), (o = (a = this.props).onBlur) == null || o.call(a);
      });
    }, e = this.createInput({
      value: this.value,
      inputType: this.inputType,
    ...
    if (this.inputElement = e.querySelector("input.number-setting-input"), !this.props.suffix || this.props.suffix === "none")
      return e;
    e.classList.add("suffix-wrapper"), this.inputElement && (this.inputElement.style.paddingRight = "35px");
    const i = document.createElement("span");
    return i.className = "suffix-label", i.textContent = this.props.suffix, e.appendChild(i), e;
  }
  setValue(t) {
    const e = this.validateValue(t);
    super.setValue(e), this.inputElement && this.inputElement.value !== String(e) && (this.inputElement.value = String(e ?? ""));
  }
  validateValue(t) {
    (t == null || isNaN(t)) && (t = this.props.default ?? 0);
    const e = this.props.minValue ?? Number.MIN_SAFE_INTEGER, i = this.props.maxValue ?? Number.MAX_SAFE_INTEGER;
    let s = Number(t);
...
class F extends g {
  constructor(t = {}) {
    super(t), this.inputType = "button", this.previewWrapper = null, this.previewEl = null, this.emptyStateEl = null, this.props.delete === void 0 && (this.props.delete = !0), !this.value && t.defaultUrl && t.defaultUrl !== "" && (this.value = t.defaultUrl);
  }
  ...
  draw() {
    ...
    t.className = "upload-setting-wrapper", t.setAttribute("data-setting-id", this.id), t.addEventListener("focusout", (l) => {
      var r;
      l.relatedTarget && t.contains(l.relatedTarget) || (r = this.onBlur) == null || r.call(this, this.value ?? "");
    }, !0);
    ...
    if (n.className = "preview-placeholder", n.innerHTML = q, this.previewWrapper = document.createElement("div"), this.previewWrapper.className = "preview-wrapper", this.previewEl = document.createElement("img"), this.previewEl.className = "upload-preview", this.props.delete) {
      ...
      l.className = "delete-button", l.type = "button", l.title = "Delete image", l.innerHTML = D, this.previewWrapper.appendChild(l), l.onclick = (r) => {
        var h;
    ...
    return o.type = "file", o.accept = "image/*", o.style.display = "none", i.appendChild(n), i.appendChild(this.previewWrapper), i.appendChild(a), t.appendChild(i), t.appendChild(o), s ? this.updatePreviewState(this.value) : this.updatePreviewState(null), a.onclick = () => {
      ...
      if (l)
        try {
          ...
          const C = this.props.requestMethod || "POST", v = { ...this.props.requestHeaders || {} }, x = await (await fetch(p, {
            method: C,
            body: d,
            headers: v
          })).json();
          ...
          if (this.props.parseResponse)
            E = this.props.parseResponse(x);
          else if (E = (h = x == null ? void 0 : x.data) == null ? void 0 : h.url, !E)
            throw new Error(
              "No URL found in response. Provide a parseResponse if needed."
            );
          console.log("Final URL:", E), this.value = E, this.updatePreviewState(E), (u = this.onChange) == null || u.call(this, E);
        } catch (d) {
...
class rt extends b {
  ...
  // Optional: Add a method to get the mobile value
  getMobileValue() {
    return this.mobileValue;
  }
}
...
class ht extends b {
  ...
  constructor(t = {}) {
    const e = {
      ...
      icon: t.icon || z,
      ...t
      // Apply provided props, potentially overriding defaults
    };
    super(e), this.inputType = "number", this.mobileValue = t.mobile;
  }
  // Optional: Add a method to get the mobile value if needed by consuming app
  getMobileValue() {
    return this.mobileValue;
  }
}
...
class ct extends g {
  constructor(t = {}) {
    if (super(t), this.inputType = "select", this._options = [], this.isOpen = !1, this.selectedOptionIndex = null, this.optionsListEl = null, this.svgContainer = null, this.buttonEl = null, this.isLoading = !1, this.container = null, this.hasInitializedOptions = !1, this.selectedOptionIndex = null, t.default !== void 0 && (this.value = t.default), this.initializeOptions(t), !t.getOptionsAsync && this.value !== void 0) {
      ...
      e !== -1 && (this.selectedOptionIndex = e);
    }
    t.onChange && this.setOnChange(t.onChange), t.detectChange && (this.detectChangeCallback = t.detectChange);
  }
  initializeOptions(t) {
    this.hasInitializedOptions && !t.getOptionsAsync || (this._options = [], t.options && (this._options = [...t.options]), t.getOptions && this._options.push(...t.getOptions()), t.getOptionsAsync ? this.isLoading = !0 : this.hasInitializedOptions = !0);
  }
  createOption(t, e) {
    const i = document.createElement("li");
  ...
  updateButtonText(t, e = !1) {
    ...
    if (i)
      if (e)
        i.textContent = t || "Error", this.buttonEl.classList.add("error"), this.buttonEl.setAttribute("disabled", "true"), this.optionsListEl && this.optionsListEl.classList.remove("open"), this.svgContainer && (this.svgContainer.style.display = "none");
      else if (this.isLoading)
        i.textContent = this.props.loadingText || "Loading options...", this.buttonEl.classList.remove("error"), this.buttonEl.removeAttribute("disabled"), this.svgContainer && (this.svgContainer.style.display = "");
      else {
        const s = this.selectedOptionIndex !== null && this._options && this.selectedOptionIndex >= 0 && this.selectedOptionIndex < this._options.length ? this._options[this.selectedOptionIndex] : null, n = s && typeof s.name == "string" ? s.name : "Select an option";
...
class ut extends g {
  ...
  draw() {
    const t = document.createElement("div");
    t.className = `setting-container ${this.props.className || ""}`, t.id = this.id;
    const e = document.createElement("div");
    e.className = "setting-label-input-wrapper";
    const i = document.createElement("label");
...
class k extends f {
  ...
  updateSettingsFromTab(t) {
    ...
    e && (this.settings = e.settings, this.tabValues[t] = this.getTabValues(e), this.initialValues = {
      ...
      i instanceof k && (i._parentTabsSettings = this);
  ...
  connectChildChangeHandlers() {
    ...
    Object.values(this.settings).forEach((e) => {
      ...
      Object.entries(e.settings).forEach(([i, s]) => {
        (s instanceof f || s instanceof g) && (s instanceof k ? (s._parentTabsSettings = this, s.setOnChange((n) => {
          Promise.resolve().then(() => {
  ...
  updateInitialValues() {
    !this.tabs || this.tabs.length === 0 || (this.updateTabValues(), this.initialValues = {
      ...
      Object.values(t.settings).forEach((e) => {
        e instanceof k && e.updateInitialValues();
      });
    }));
  }
  setOnChange(t) {
  ...
  getTabValues(t) {
    ...
    return Object.entries(t.settings).forEach(([i, s]) => {
      s instanceof k || s instanceof f ? e[i] = s.getValues() : (s instanceof g, e[i] = s.value);
    }), e;
  }
  /**
   * Clone this TabsSettings and all its nested settings
   */
  clone() {
    const t = this.tabs.map((i) => {
      const s = {};
      return Object.entries(i.settings).forEach(([n, a]) => {
        typeof a.clone == "function" ? s[n] = a.clone() : (console.warn(`Setting with key '${n}' in tab '${i.id}' does not have a clone method. Copying reference.`), s[n] = a);
      }), {
        ...i,
        settings: s
      };
        ...
        if (this.updateSettingsFromTab(this.activeTabId), this.tabsContainer) {
          ...
          s == null || s.forEach((n) => {
            if (n instanceof HTMLElement) {
              const a = this.tabs.find((o) => o.label === n.textContent);
              n.classList.toggle("active", (a == null ? void 0 : a.id) === this.activeTabId);
            }
          });
        }
